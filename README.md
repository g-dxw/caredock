...
这是全新的一期技术骨架。R0已经验证Tauri、Gateway/Command、Rust分层、bundled SQLite、事务和备份能力；M0—M2已经加入正式数据库连接、migration runner、升级前备份及A/B上下文正式表。

- 不包含正式业务页面。
- 正式数据库文件为`anyangtai.sqlite`，与`r0-probe.sqlite`完全隔离。
- 当前已实现`schema_migrations`和`0001—0005`，共22张业务表；`0006—0012`尚未开始。
